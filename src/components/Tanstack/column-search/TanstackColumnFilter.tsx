...
import { type Vehicle, createData } from "./makeData";
import {
	type ColumnDef,
	flexRender,
	getCoreRowModel,
...
const TanstackColumnFilter = () => {
	...
	const [data, _setData] = useState(() => createData(10));
		...
		<div>
			<TdsTable>
				...
				{table.getHeaderGroups().map((headerGroup) => (
					<TdsTableHeader key={`${headerGroup.id}-input`}>
						{headerGroup.headers.map((header) => {
							return (
								<TdsHeaderCell disablePadding key={`${header.id}-input`} cellKey={header.id}>
